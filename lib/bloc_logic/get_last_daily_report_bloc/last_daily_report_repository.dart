import 'dart:convert';

import 'package:dio/dio.dart';

import '../../constant/api.dart';
import '../../http/api_client.dart';
import '../../local_storage/my_local_storage.dart';
import '../../models/daily_report_model.dart';
import '../../models/error_model.dart';
import '../../models/success_model.dart';
import '../../utils/exceptions/service_exception.dart';
import '../../utils/logger.dart';

class LastDailyReportRepository {
  static Future<DailyReportModel> getLastDailyReport({String? token}) async {
    final response = await DioClient().getRequest(
        url: Api.baseurl + Api.getUserLastDailyReport,
        token: MyLocalStorage().getToken());
    var responseStatus = SuccessModel.fromJson(response.data);
    if (responseStatus.status == true) {
      var responseData = DailyReportModel.fromJson(response.data);
      return responseData;
    } else {
      Logger.println("Get Last Daily Report Error :::: ${response.data}");
      var responseStatus = ErrorModel.fromJson(response.data);
      throw ServiceException(
          message: responseStatus.error![0].error.toString());
    }
  }
}
